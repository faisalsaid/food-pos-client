...
export default function PaymentModal({ isOpen, closeModel, content }) {
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={closeModel} shouldCloseOnEsc={false} shouldCloseOnOverlayClick={false} contentLabel="Payment Modal" style={customStyles}>
      <Formik enableReinitialize onReset={handleReset} initialValues={initialValues} onSubmit={handleSubmit} validationSchema={validationSchema}>
        {(paymentForm) => {
          ...
          return (
            <div className="w-[500px] ">
              <div className="flex items-center gap-2  border-b pb-2 ">
                ...
                <button type="button" onClick={closeModel} className="bg-red-600 p-1 text-white rounded-md">
                  <AiOutlineClose />
                </button>
              </div>
              <div className="py-8 flex flex-col gap-5">
                <div>
                  ...
                  <div className="flex gap-2 border rounded-lg p-2 items-center">
                    <span>$</span>
                    <Field className="flex-1 text-center text-2xl font-bold outline-none" type="number" placeholder="eg: 100.00" name="amount" />
                  </div>
                  <ErrorMessage name="amount" component={ErrorCompsFormik} />
                </div>
              ...
              <div className="py-2 flex gap-2">
                <button className="bg-red-500 text-white py-1 px-2 rounded-md">Cancel</button>
                <button
                  onClick={closeModel}
